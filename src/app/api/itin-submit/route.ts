...
async function forwardToTaskboard(
  ...
  const payload = {
    ...
    services: ["ITIN"],
    source: "itin-kiosk",
    send_id: data.send_id?.trim() || undefined,
    metadata: {
      lead_type: "itin-kiosk",
      raw: {
        send_id: data.send_id?.trim() || undefined,
      },
    },
    itin: {
      ...
      countryOfBirth: data.countryOfBirth?.trim() || undefined,
      cityOfBirth: data.cityOfBirth?.trim() || undefined,
      countryOfCitizenship: data.countryOfCitizenship?.trim() || undefined,
      city: data.city,
      addressUsa: data.addressUsa.trim() || undefined,
      aptNumber: data.aptNumber?.trim() || undefined,
      zipCode: data.zipCode?.trim() || undefined,
      addressHomeCountry: data.addressHomeCountry.trim() || undefined,
      homeCountry: data.homeCountry?.trim() || undefined,
      homeCity: data.homeCity?.trim() || undefined,
      homeAddress: data.homeAddress?.trim() || undefined,
      usEntryDate: data.usEntryDate || undefined,
      ...
      passportCountry: data.passportCountry?.trim() || undefined,
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const data: ItinPayload = {
      ...
      send_id: ((formData.get("send_id") as string) || (formData.get("sendId") as string) || "")
        .trim()
        .slice(0, 100),
    };
